...
type DurationOption = 15 | 30 | 45 | 60 | 90;
type WizardStep =
  | "duration"
  | "format"
  | "slot"
...
type TelegramBotRuntime = {
  bot: Bot<AppContext>;
  webhookHandler: (...args: any[]) => Promise<unknown>;
};
...
const pendingRescheduleByUser = new Map<string, string>();
const actionLocks = new Map<string, number>();
const rateLimitMap = new Map<string, number>();
let meetingSettingsCache: { value: MeetingSettings; cachedAt: number } | null = null;
...
function acquireActionLock(key: string): boolean {
  const now = Date.now();
...
function releaseActionLock(key: string): void {
  actionLocks.delete(key);
}

function isRateLimited(userTelegramId: string, actionKey: string, intervalMs: number): boolean {
...
async function getCachedMeetingSettings(): Promise<MeetingSettings> {
  const now = Date.now();
...
function buildBotInfoFromToken(botToken: string): {
  id: number;
  is_bot: true;
  first_name: string;
  username: string;
...
function isIgnorableCallbackAnswerError(error: unknown): boolean {
...
async function safeAnswerCallbackQuery(ctx: AppContext): Promise<void> {
  try {
    ...
    const callbackId =
      isRecord(ctx.callbackQuery) && typeof ctx.callbackQuery.id === "string"
...
function parseDraftPayload(value: Prisma.JsonValue): DraftPayload {
  if (!isRecord(value)) {
    return {};
  }
  ...
  if (typeof value.durationMinutes === "number") {
    const candidate = value.durationMinutes as number;
    if (DURATION_OPTIONS.includes(candidate as DurationOption)) {
      payload.durationMinutes = candidate as DurationOption;
    }
  }
  ...
  if (typeof value.description === "string" || value.description === null) {
    payload.description = value.description as string | null;
  }
...
function formatTimeMoscow(date: Date): string {
  return new Intl.DateTimeFormat("ru-RU", {
    timeZone: "Europe/Moscow",
    hour: "2-digit",
    minute: "2-digit"
...
function formatReview(payload: DraftPayload): string {
  const lines: string[] = ["Проверьте данные заявки:"];
...
async function expireOldDrafts(userId: string): Promise<void> {
  await prisma.meetingRequestDraft.updateMany({
    where: {
      userId,
...
async function getActiveDraft(userId: string): Promise<MeetingRequestDraft | null> {
  await expireOldDrafts(userId);

  return prisma.meetingRequestDraft.findFirst({
    where: {
      userId,
      status: DraftStatus.ACTIVE,
...
async function discardActiveDrafts(userId: string): Promise<void> {
  await prisma.meetingRequestDraft.updateMany({
    where: {
      userId,
      status: DraftStatus.ACTIVE
...
async function createDraft(user: User): Promise<MeetingRequestDraft> {
  const payload: DraftPayload = {
...
): Promise<MeetingRequestDraft> {
  const prevStep = draft.currentStep as WizardStep;

  const updated = await prisma.meetingRequestDraft.update({
    where: { id: draft.id },
    data: {
...
function getNextStep(payload: DraftPayload): WizardStep {
...
function getPreviousStep(currentStep: WizardStep, payload: DraftPayload): WizardStep {
...
function formatKeyboard(): InlineKeyboard {
...
function slotsKeyboard(slots: Slot[]): InlineKeyboard {
  const keyboard = new InlineKeyboard();
...
function reviewKeyboard(payload: DraftPayload): InlineKeyboard {
  const keyboard = new InlineKeyboard()
...
function approvalKeyboard(meetingRequestId: string): InlineKeyboard {
  return new InlineKeyboard()
...
function requestActionsKeyboard(request: Pick<MeetingRequest, "id" | "status">): InlineKeyboard | null {
  if (
...
function rescheduleSlotsKeyboard(meetingRequestId: string, slots: Slot[]): InlineKeyboard {
  const keyboard = new InlineKeyboard();
...
function formatApprovalCard(meetingRequest: MeetingRequest, requester: User): string {
  const lines: string[] = ["🟡 Новая заявка на подтверждение."];
...
function resolveAdminChatId(): number | string | null {
...
function isAdminActor(ctx: AppContext): boolean {
...
function adminSettingsKeyboard(): InlineKeyboard {
  return new InlineKeyboard()
...
function formatAdminSettingsMessage(settings: MeetingSettings): string {
  return [
...
async function showAdminSettingsPanel(ctx: AppContext): Promise<void> {
  const settings = await getCachedMeetingSettings();
...
async function changeAdminSetting(ctx: AppContext, key: string, delta: number): Promise<void> {
  const current = await getCachedMeetingSettings();
  let updated: MeetingSettings;

  switch (key) {
    case "workday_start_hour":
      updated = await patchMeetingSettings({ workdayStartHour: current.workdayStartHour + delta });
      break;
    case "workday_end_hour":
      updated = await patchMeetingSettings({ workdayEndHour: current.workdayEndHour + delta });
      break;
    case "slot_buffer_minutes":
      updated = await patchMeetingSettings({ slotBufferMinutes: current.slotBufferMinutes + delta });
      break;
    case "slot_limit":
      updated = await patchMeetingSettings({ slotLimit: current.slotLimit + delta });
      break;
    case "slot_horizon_days":
      updated = await patchMeetingSettings({ slotHorizonDays: current.slotHorizonDays + delta });
      break;
    case "slot_min_lead_hours":
      updated = await patchMeetingSettings({ slotMinLeadHours: current.slotMinLeadHours + delta });
      break;
    default:
...
async function showHistory(ctx: AppContext, user: User): Promise<void> {
  const requests = await prisma.meetingRequest.findMany({
...
async function showConsent(ctx: AppContext): Promise<void> {
  const keyboard = new InlineKeyboard().text("Согласен(на)", ACTION.CONSENT_ACCEPT);

  await ctx.reply(
    "Перед записью нужно согласиться на обработку персональных данных. Нажмите кнопку ниже для продолжения.",
    {
...
async function showStartMenu(ctx: AppContext, user: User): Promise<void> {
...
async function startWizard(ctx: AppContext, user: User): Promise<void> {
...
async function restoreDraft(ctx: AppContext, user: User, draft: MeetingRequestDraft): Promise<void> {
  const payload = parseDraftPayload(draft.payload);
  const currentStep = draft.currentStep as WizardStep;
  ...
  await appendActionLog({
    ...
    actionType: JournalActionType.DRAFT_RESTORED,
    details: {
      step: currentStep
    }
  });
...
async function cancelWizard(ctx: AppContext, user: User): Promise<void> {
...
async function handleBackAction(ctx: AppContext, user: User): Promise<void> {
  const draft = await getActiveDraft(user.id);
  if (!draft) {
...
async function submitMeetingRequest(
  ...
  await prisma.meetingRequestDraft.update({
    where: { id: draft.id },
    data: {
      status: DraftStatus.DISCARDED
    }
  ...
  const submitLines: string[] = ["🟡 Заявка отправлена на подтверждение.", "Мы сообщим, как только администратор примет решение.", ""];
...
async function notifyUserAboutApprovalResult(input: {
  ...
  const lines: string[] = input.approved ? ["✅ Встреча подтверждена.", ""] : ["❌ Заявка отклонена.", ""];
...
async function createCalendarEventForMeetingRequest(request: MeetingRequest): Promise<CalendarEventCreateResult> {
...
async function approveMeetingRequest(ctx: AppContext, meetingRequestId: string, adminActorId: string): Promise<void> {
  ...
  let calendarEventResult: CalendarEventCreateResult;
  ...
  try {
    ...
    await appendActionLog({
      ...
      actionType: JournalActionType.INTEGRATION_ERROR,
      details: {
        stage: "approval",
        error: errorMessage
      },
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.actionLog.create({
      data: {
        ...
        actorRole: JournalActorRole.SYSTEM,
        actorId: "calendar_sync",
        actionType: JournalActionType.CALENDAR_EVENT_CREATED,
        details: {
          calendar_event_id: calendarEventResult.googleCalendarEventId
        },
        result: "ok"
  ...
  try {
    const cancelledJobsCount = await cancelPendingApprovalReminderJobs(request.id);
    logEvent({
...
async function cancelMeetingRequestByUser(ctx: AppContext, user: User, meetingRequestId: string): Promise<void> {
  const request = await prisma.meetingRequest.findUnique({
    where: { id: meetingRequestId },
      ...
      if (request.calendarEvent) {
        ...
        await tx.actionLog.create({
          data: {
            ...
            actorRole: JournalActorRole.SYSTEM,
            actorId: "calendar_sync",
            actionType: JournalActionType.CALENDAR_EVENT_DELETED,
            details: {
...
async function requestRescheduleByUser(ctx: AppContext, user: User, meetingRequestId: string): Promise<void> {
  const request = await prisma.meetingRequest.findUnique({
    where: { id: meetingRequestId }
  });
  ...
  await appendActionLog({
    ...
    actorRole: JournalActorRole.USER,
    actorId: user.telegramId,
    actionType: JournalActionType.RESCHEDULE_REQUESTED,
    details: {
      source: "telegram_user_action"
    }
  });
...
async function completeRescheduleByUser(
  ctx: AppContext,
  user: User,
  meetingRequestId: string,
  slotIndex: number
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.actionLog.create({
        data: {
          ...
          actorRole: JournalActorRole.SYSTEM,
          actorId: "calendar_sync",
          actionType: JournalActionType.CALENDAR_EVENT_UPDATED,
          details: {
            calendar_event_id: calendarEventResult.googleCalendarEventId
          },
          result: "ok"
...
async function tryHandlePendingRejectionComment(ctx: AppContext, user: User): Promise<boolean> {
...
async function handleTextInput(ctx: AppContext, user: User): Promise<void> {
...
function configureDryRunApi(bot: Bot<AppContext>): void {
  let messageId = 1;
  const ok = <T>(result: T): { ok: true; result: T } => ({ ok: true, result });

  bot.api.config.use(async (_prev, method, payload) => {
    if (method === "getMe") {
      return ok(bot.botInfo) as never;
    }
    ...
    if (method === "sendMessage" || method === "editMessageText") {
      const chatId = Number((payload as { chat_id?: number | string }).chat_id ?? 1);
...
export function createTelegramBotRuntime(
  ...
  runtimeAdminTelegramId = options.adminTelegramId === undefined ? getApprovalConfig().adminTelegramId : options.adminTelegramId;
  pendingRejectionCommentByAdmin.clear();
  pendingRescheduleByUser.clear();
  actionLocks.clear();
  rateLimitMap.clear();

  const bot = new Bot<AppContext>(options.botToken, {
    botInfo: buildBotInfoFromToken(options.botToken) as never
  });
  ...
  bot.catch((error) => {
    const nestedErrorMessage =
      isRecord(error) && isRecord(error.error) && typeof error.error.message === "string"
        ? error.error.message
        : null;
  ...
  bot.callbackQuery(/^dur:(15|30|45|60|90)$/, async (ctx) => {
    ...
    const value = Number(ctx.match[1]) as DurationOption;
    const payload = parseDraftPayload(draft.payload);
    payload.durationMinutes = value;
    payload.slotStartAt = undefined;
    payload.slotEndAt = undefined;

    const nextStep: WizardStep = payload.format ? "slot" : "format";
  ...
  bot.callbackQuery(/^fmt:(ONLINE|OFFLINE)$/, async (ctx) => {
    ...
    const value = ctx.match[1] as MeetingFormat;
    const payload = parseDraftPayload(draft.payload);
  ...
  bot.callbackQuery(/^edit:(duration|format|slot|topic|description|email|first_name|last_name|location)$/, async (ctx) => {
    ...
    const payload = parseDraftPayload(draft.payload);
    const step = ctx.match[1] as WizardStep;
  ...
  bot.callbackQuery(APPROVAL_ACTION_PATTERN, async (ctx) => {
    ...
    await ctx.reply(
      `Введите комментарий к отклонению для заявки ${requestCode}.\nОтправьте «-», если без комментария.`
    );
  });
...
export function getTelegramRuntime(): TelegramBotRuntime | null {
...
export async function ensureWizardStateForUser(userTelegramId: string): Promise<{
  draft: MeetingRequestDraft | null;
